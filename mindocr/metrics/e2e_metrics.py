...
def get_score(gt_dir, pred_dict):
    allInputs = 1
...
def combine_results(all_data, rec_flag=True):
    ...
    final = {
        ...
        'det_fscore': f_score,
        'seqerr': seqerr,
        'e2e_recall': recall_e2e,
        'e2e_precision': precision_e2e,
        'e2e_fscore': f_score_e2e
    }
    return final
...
class PGNetMetric:
    def __init__(
        ...
        **kwargs
        ...
        self.metric_names = ["total_num_gt", "total_num_det", "seqerr",
                             "global_accumulative_recall", "hit_str_count",
                             "det_precision", "det_recall", "det_fscore",
                             "e2e_precision", "e2e_recall", "e2e_fscore"]
    ...
    def update(self, *inputs):
        ...
        tmp_gt_strs = gt[2][0]
        ignore_tags = gt[3][0]
        if isinstance(gt_polyons, Tensor):
            gt_polyons = gt_polyons.asnumpy()
            tmp_gt_strs = tmp_gt_strs.asnumpy()
            ignore_tags = ignore_tags.asnumpy()

        gt_strs = []
        for tmp_list in tmp_gt_strs:
            t = ""
        ...
        result = get_score(gt_info_list, e2e_info_list)
