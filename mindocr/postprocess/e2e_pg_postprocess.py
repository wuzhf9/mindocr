...
class PGNetPostProcess:
    ...
    def __call__(self, preds, **kwargs):
        ...
        if isinstance(f_score, Tensor):
            f_score = f_score[0].asnumpy()
            f_border = f_border[0].asnumpy()
            f_char = f_char[0].asnumpy()
            f_direction = f_direction[0].asnumpy()
        else:
            f_score = f_score[0]
            f_border = f_border[0]
            f_char = f_char[0]
            f_direction = f_direction[0]

        shape_list = kwargs["shape_list"]
        if isinstance(shape_list, Tensor):
            shape_list = shape_list.asnumpy().tolist()

        src_h, src_w, ratio_h, ratio_w = shape_list[0]
        instance_yxs_list, seq_strs = generate_pivot_list_fast(
            f_score, f_char, f_direction,
            self.lexicon_table, self.score_thresh, self.point_gather_mode 
        )
        poly_list, keep_str_list = restore_poly(
            instance_yxs_list, seq_strs, f_border,
            ratio_w, ratio_h, src_w, src_h, self.valid_set
        )

        return {"points": poly_list, "texts": keep_str_list}
